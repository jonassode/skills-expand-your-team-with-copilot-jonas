...
def get_activities(
    ...
    - difficulty: Filter activities by difficulty level (e.g., 'Beginner', 'Intermediate', 'Advanced', 'no-level' for activities without difficulty)
    """
    ...
    if difficulty:
        if difficulty == "no-level":
            # "no-level" means activities with no difficulty specified
